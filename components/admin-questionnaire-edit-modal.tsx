...
import { AdminQuestionnaireQuestion, UpdateQuestionRequest, UpdateCostItemRequest } from '@/types/admin-questionnaire';
import { useUpdateQuestion, useGetCostItemsByQuestion, useCreateCostItem, useUpdateCostItem, useDeleteCostItem } from '@/hooks/useAdminQuestionnaire';

interface AdminQuestionnaireEditModalProps {
    isOpen: boolean;
    onClose: () => void;
    question: AdminQuestionnaireQuestion;
    sectionId: number;
    subsectionId: number;
}

const AdminQuestionnaireEditModal: React.FC<AdminQuestionnaireEditModalProps> = ({
    isOpen,
    onClose,
    question,
    sectionId, // eslint-disable-line @typescript-eslint/no-unused-vars
    subsectionId
}) => {
    const updateQuestion = useUpdateQuestion();
    const { data: existingCostItems } = useGetCostItemsByQuestion(question.categoryQuestionID);
    ...
    const [costItems, setCostItems] = useState<Array<UpdateCostItemRequest & { costItemID?: number }>>([]);
    ...
    useEffect(() => {
        if (existingCostItems && existingCostItems.length > 0) {
            setCostItems(
                existingCostItems.map(item => ({
                    costItemID: item.costItemID,
                    item: item.item,
                    estimatedAverage: item.estimatedAverage,
                    subsectionID: item.subsectionID,
                    categoryQuestionID: item.categoryQuestionID || question.categoryQuestionID
                }))
            );
        } else if (question.costItems && question.costItems.length > 0) {
            // Fallback to question.costItems if API data not available
            setCostItems(
                question.costItems.map(item => ({
                    costItemID: item.costItemID,
                    item: item.item,
                    estimatedAverage: item.estimatedAverage,
    ...
    }, [existingCostItems, question, subsectionId]);
    ...
    const handleRemoveCostItem = async (index: number) => {
        const item = costItems[index];
        // If it's an existing cost item (has ID), delete it via API
        if (item.costItemID) {
            try {
                await deleteCostItem.mutateAsync(item.costItemID);
            } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Handle cost items - create new ones and update existing ones
            for (const costItem of costItems) {
                if (costItem.costItemID) {
                    // Update existing cost item
                    await updateCostItem.mutateAsync({
                        costItemId: costItem.costItemID!,
                        data: {
                            subsectionID: costItem.subsectionID,
                            categoryQuestionID: costItem.categoryQuestionID || null,
                            item: costItem.item.trim(),
                            estimatedAverage: costItem.estimatedAverage || null
                        }
                    });
                } else if (costItem.item.trim()) {
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 lg:ml-6 max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200">
                        ...
                        <button
                            type="submit"
                            disabled={updateQuestion.isPending || createCostItem.isPending || updateCostItem.isPending || deleteCostItem.isPending}
                            className="px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            {(updateQuestion.isPending || createCostItem.isPending || updateCostItem.isPending || deleteCostItem.isPending) ? 'Saving...' : 'Save Changes'}
                        </button>
                    </div>
                </form>
            </div>
...
export default AdminQuestionnaireEditModal;
