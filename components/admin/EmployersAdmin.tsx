...
import { FaBuilding, FaEdit, FaFileExcel, FaPlus, FaRedo, FaSearch } from 'react-icons/fa';
...
import type { Employer, EmployerFormDraft, EmployerId, EmployerUpdatePayload } from '@/types/employers';
import { useEmployerMutations, useEmployersList } from '@/hooks/useEmployers';
import {
  getEmployer,
  listEmployerAccreditationBodies,
  setEmployerAccreditationBodies,
} from '@/api/employers';
...
function emptyDraft(createdBy: number): EmployerFormDraft {
  return {
    employerName: '',
    subSector: '',
    physicalAddress1: '',
...
function recordToDraft(p: Employer, fallbackCreatedBy: number): EmployerFormDraft {
...
function employerKey(p: Employer): string {
  const id = p.employerId;
  if (id != null && String(id)) return String(id);
  return `${p.employerName ?? 'row'}-${p.emailAddress ?? ''}-${p.sdlNumber ?? ''}`;
}
...
export default function EmployersAdmin() {
  ...
  const { data: employers = [], isLoading, error, refetch, isFetching } = useEmployersList();
  const { createMutation, updateMutation } = useEmployerMutations();

  const [filter, setFilter] = useState('');
  const [formOpen, setFormOpen] = useState(false);
  const [formMode, setFormMode] = useState<'create' | 'edit'>('create');
  const [editingId, setEditingId] = useState<EmployerId | null>(null);
  const [draft, setDraft] = useState<EmployerFormDraft>(() => emptyDraft(createdBy));
  ...
  const handleExportExcel = useCallback(() => {
    ...
    exportRowsToExcel('employers', rows, 'Employers');
  }, [filtered]);
  ...
  const openEdit = useCallback(
    async (p: Employer) => {
      ...
      let full: Employer = p;
      try {
        full = await getEmployer(id);
      } catch {
        /* list row may be enough */
      }
      let accreditationBodyIds: number[] = [];
      try {
        const links = await listEmployerAccreditationBodies(id);
  ...
  const submitForm = useCallback(async () => {
    ...
    try {
      ...
      if (formMode === 'create') {
        ...
        const newId = created.employerId;
        if (newId != null) {
          await setEmployerAccreditationBodies(newId, { accreditationBodyIds: ids });
        }
        ...
        await setEmployerAccreditationBodies(editingId, { accreditationBodyIds: ids });
      }
      closeForm();
    } catch (e) {
  ...
  return (
    <div className="mx-auto max-w-[1500px] bg-gradient-to-b from-slate-50/70 to-white pb-8">
      <div className="mb-4 flex items-center gap-2 px-4 text-sm text-gray-600 lg:px-6">
        ...
        <span>/</span>
        <span className="font-medium text-gray-900">Employers</span>
      </div>
      ...
      <div className="mb-4 rounded-3xl border border-slate-200/80 bg-white px-5 py-4 shadow-[0_10px_30px_rgba(15,23,42,0.06)] lg:mx-6">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-hwseta-green">Employer setup</p>
            <h1 className="mt-1 text-2xl font-bold text-gray-900">Employers</h1>
            <p className="mt-1 text-sm text-gray-600">
              Manage employer records, contact details, and accreditation.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2 text-xs font-medium text-gray-600">
            <span className="rounded-full bg-hwseta-green/10 px-3 py-1.5 text-hwseta-green">Employers</span>
            <span className="rounded-full bg-emerald-50 px-3 py-1.5 text-emerald-700">{employers.length} records</span>
          </div>
        </div>
      </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.6)]">
            <div className="flex flex-col gap-3">
              ...
              <div className="max-w-xl">
                <label className="sr-only">Search employers</label>
            ...
            <div className="mt-5 flex min-h-[280px] flex-col overflow-hidden rounded-2xl border border-slate-200 shadow-sm">
                ...
                <table className="min-w-full whitespace-nowrap divide-y divide-gray-200 text-sm">
                  <thead className="sticky top-0 z-10 bg-hwseta-green text-white shadow-sm">
                    <tr className="text-left text-xs font-semibold uppercase tracking-wide">
                      <th className="px-5 py-3 text-white/90">Employer Name</th>
                  ...
                  <tbody className="divide-y divide-gray-100 bg-white">
                    {isLoading ? (
                      ...
                      pagedEmployers.map((p, index) => (
                        ...
                        >
                          <td className="px-5 py-4 font-semibold text-gray-900">{p.employerName ?? '—'}</td>
              ...
              {totalGridCount > 0 ? (
                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                  <p>
                    <span className="font-semibold text-slate-800">Employers</span> found {totalGridCount}
                  </p>
                  <p>
                    Showing{' '}
                    <span className="font-semibold text-slate-800">
                      {gridPageStart + 1}–{Math.min(gridPageStart + pagedEmployers.length, totalGridCount)}
                    </span>{' '}
      ...
      <EmployerFormModal
        open={formOpen}
        onClose={closeForm}
        title={formMode === 'create' ? 'Add employer' : 'Edit employer'}
        draft={draft}
        setDraft={setDraft}
        formError={formError}
        onSubmit={submitForm}
...
function EmployerFormModal(props: {
  open: boolean;
  onClose: () => void;
  title: string;
  draft: EmployerFormDraft;
  setDraft: Dispatch<SetStateAction<EmployerFormDraft>>;
  ...
  const set = <K extends keyof EmployerFormDraft>(key: K, v: EmployerFormDraft[K]) => {
    setDraft((d) => ({ ...d, [key]: v }));
  };
                  ...
                  <section>
                    ...
                    <div className="mt-3 space-y-3">
                      <FormToggleSwitch
                        ...
                        label="ETQA"
                        description="Employer is ETQA-accredited."
                      />
                      {draft.etqa && (
